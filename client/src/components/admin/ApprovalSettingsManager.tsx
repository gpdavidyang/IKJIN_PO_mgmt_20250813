/**
 * Approval Settings Manager - Enhanced for System Admin Tab
 * 시스템 관리 탭의 승인 권한 관리 컴포넌트
 */
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold">승인 권한 관리</h3>
          <p className="text-sm text-gray-600">사용자 역할별 승인 권한과 한도를 관리합니다</p>
        </div>
        
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button size="sm" onClick={() => { resetForm(); setEditingAuthority(null); }}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {authorities.filter(auth => auth.isActive).length}
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">승인 권한 목록</CardTitle>
                ...
                <div key={authority.id} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex-1">
                    ...
                    {authority.description && (
                      <p className="text-sm text-gray-600 mt-1">{authority.description}</p>
                    )}
