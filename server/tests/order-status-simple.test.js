...
describe('발주서 상태 관리 로직 테스트', () => {
  ...
  describe('상태 전환 로직', () => {
    it('draft → created 전환이 올바르게 작동해야 함', () => {
      ...
      // 상태 전환 로직
      const getNextStatus = (current, actionType) => {
    ...
    it('잘못된 상태 전환은 거부되어야 함', () => {
      ...
      const isValidTransition = (current, actionType) => {
        ...
        return validTransitions[current]?.includes(actionType) || false;
      };
  ...
  describe('히스토리 로깅 로직', () => {
    it('상태 변경 시 올바른 히스토리 엔트리가 생성되어야 함', () => {
      const createHistoryEntry = (orderId, userId, action, changes) => {
        return {
          orderId,
          userId,
          action,
  ...
  describe('워크플로우 검증', () => {
    it('전체 워크플로우가 올바른 순서로 진행되어야 함', () => {
      ...
      const validateWorkflowOrder = (currentStatus, nextStatus) => {
