...
var ExcelAutomationService = class {
  ...
  static async validateVendorsFromExcel(filePath) {
    ...
    try {
      ...
      const validationResults3 = await validateMultipleVendors(vendorData);
      const validVendors = [];
      const invalidVendors = [];
      for (const result of validationResults3.vendorValidations) {
...
// server/routes/excel-smart-upload.ts
import { Router as Router33 } from "express";
import multer6 from "multer";
import { z as z10 } from "zod";
import crypto2 from "crypto";

// server/services/smart-upload-service.ts
init_db();
init_schema();
import * as XLSX10 from "xlsx";
import { eq as eq30, and as and23 } from "drizzle-orm";
...
var ValidationService = class {
   ...
   */
  async applyCorrections(sessionId, corrections, userId) {
    const session2 = await db.query.validationSessions.findFirst({
      where: and20(
        eq27(validationSessions.id, sessionId),
        eq27(validationSessions.userId, userId)
      )
    });
    if (!session2) {
    ...
    for (const correction of corrections) {
      await db.update(validationResults).set({
        validatedValue: String(correction.value),
        validationStatus: "valid",
        errorMessage: null,
        confidence: 100
      }).where(and20(
        eq27(validationResults.sessionId, sessionId),
        eq27(validationResults.rowIndex, correction.rowIndex),
        eq27(validationResults.fieldName, correction.field)
      ));
    }
   ...
   */
  async getSessionErrors(sessionId, userId) {
    const session2 = await db.query.validationSessions.findFirst({
      where: and20(
        eq27(validationSessions.id, sessionId),
        eq27(validationSessions.userId, userId)
   ...
   */
  async calculateValidationSummary(sessionId) {
    const results = await db.query.validationResults.findMany({
      where: eq27(validationResults.sessionId, sessionId)
    });
    const rowStatuses = /* @__PURE__ */ new Map();
    for (const result of results) {
    ...
    await db.update(validationSessions).set({
      validItems,
      warningItems,
      errorItems
    }).where(eq27(validationSessions.id, sessionId));
  ...
  validateEmail(email) {
    ...
    return {
      isValid: false,
      suggestion: emailRegex.test(suggestion) ? suggestion : void 0
    };
  }
  /**
  ...
  validateDate(dateStr) {
    ...
    if (!parsedDate && /^\d{2}[/-]\d{2}[/-]\d{4}$/.test(dateStr)) {
      ...
      parsedDate = /* @__PURE__ */ new Date(`${parts[2]}-${parts[1]}-${parts[0]}`);
    }
   ...
   */
  validateNumeric(value, minValue, maxValue) {
    let parsed;
    if (typeof value === "number") {
      parsed = value;
    ...
    if (minValue !== void 0 && parsed < minValue) {
  ...
  validateRequired(value, fieldName) {
    if (value === null || value === void 0 || value === "" || typeof value === "string" && value.trim() === "") {
   ...
   */
  async checkDuplicates(sessionId, field, value) {
    const results = await db.query.validationResults.findMany({
      where: and20(
        eq27(validationResults.sessionId, sessionId),
        eq27(validationResults.fieldName, field),
        eq27(validationResults.originalValue, String(value))
      )
    });
   ...
   */
  async batchValidate(sessionId, rows) {
    const validationResults3 = [];
    for (const row of rows) {
    ...
    return validationResults3;
  }
};

// server/services/category-mapping-service.ts
init_db();
init_schema();
import { eq as eq28, desc as desc12, like as like4, sql as sql14 } from "drizzle-orm";
var CategoryMappingService = class {
  constructor() {
    this.cache = /* @__PURE__ */ new Map();
  }
  /**
   ...
   */
  async suggestCategories(itemName) {
   ...
   */
  async findExistingMapping(itemName) {
    const mapping = await db.query.categoryMappings.findFirst({
      where: eq28(categoryMappings.itemName, itemName),
      orderBy: [desc12(categoryMappings.confidence), desc12(categoryMappings.usageCount)]
    });
    return mapping || null;
  }
  /**
   ...
   */
  async findByPattern(itemName) {
    const similarMappings = await db.query.categoryMappings.findMany({
      where: like4(categoryMappings.itemName, `%${itemName}%`),
      orderBy: [desc12(categoryMappings.confidence), desc12(categoryMappings.usageCount)],
      limit: 5
    });
    ...
    const suggestions = similarMappings.map((mapping) => ({
   ...
   */
  applyRuleBasedMapping(itemName) {
  ...
  normalizeItemName(itemName) {
    return itemName.toLowerCase().trim().replace(/\s+/g, " ").replace(/[^\w\s가-힣]/g, "");
  }
  /**
   * Update usage count for a mapping
   */
  async updateUsageCount(mappingId) {
    await db.update(categoryMappings).set({
      usageCount: sql14`${categoryMappings.usageCount} + 1`,
      lastUsedAt: /* @__PURE__ */ new Date()
    }).where(eq28(categoryMappings.id, mappingId));
  }
  /**
   * Store new mapping for future use
   */
  async storeMapping(itemName, suggestion, userId) {
    const normalizedName = this.normalizeItemName(itemName);
    ...
    if (existing) {
      if (suggestion.confidence > Number(existing.confidence)) {
        await db.update(categoryMappings).set({
          ...
          lastUsedAt: /* @__PURE__ */ new Date()
        }).where(eq28(categoryMappings.id, existing.id));
      }
      return;
    }
  ...
  async getAllCategories() {
    const categories = await db.query.itemCategories.findMany({
      where: eq28(itemCategories.isActive, true),
      orderBy: [itemCategories.displayOrder]
    });
    const result = {
      major: [],
      middle: /* @__PURE__ */ new Map(),
      minor: /* @__PURE__ */ new Map()
    };
   ...
   */
  async learnFromCorrection(itemName, correctedCategories, userId) {
    const enhancedSuggestion = {
      ...correctedCategories,
      confidence: Math.min(correctedCategories.confidence + 10, 100)
...
// server/services/vendor-validation-service.ts
init_db();
init_schema();
import { eq as eq29, desc as desc13, sql as sql15, and as and22 } from "drizzle-orm";
var VendorValidationService = class {
  constructor() {
    this.cache = /* @__PURE__ */ new Map();
  }
  /**
   ...
   */
  async validateVendor(vendorName) {
    ...
    if (exactMatch) {
      ...
      this.cache.set(normalizedName, result2);
      return result2;
    }
   ...
   */
  async findExactMatch(vendorName) {
    const vendor = await db.query.vendors.findFirst({
      where: eq29(vendors.name, vendorName)
    });
    return vendor || null;
  }
  /**
   ...
   */
  async findMappedVendor(vendorName) {
    const mapping = await db.query.vendorMappings.findFirst({
      where: eq29(vendorMappings.originalName, vendorName),
      orderBy: [desc13(vendorMappings.confidence), desc13(vendorMappings.usageCount)]
    });
    return mapping || null;
  }
  /**
   ...
   */
  async findSimilarVendors(vendorName) {
    const allVendors = await db.query.vendors.findMany({
      where: eq29(vendors.isActive, true)
    });
    const suggestions = [];
    ...
    for (const vendor of partialMatches) {
      if (!suggestions.some((s) => s.id === vendor.id)) {
        suggestions.push({
          ...
          email: vendor.email || void 0,
          similarity: 70
          // Fixed similarity for partial matches
        });
      }
    }
    suggestions.sort((a, b) => b.similarity - a.similarity);
   ...
   */
  async findPartialMatches(vendorName) {
    const searchTerms = this.extractSearchTerms(vendorName);
    if (searchTerms.length === 0) {
      return [];
    }
    const vendors3 = await db.query.vendors.findMany({
      where: and22(
        eq29(vendors3.isActive, true),
        sql15`LOWER(${vendors3.name}) LIKE LOWER(${"%" + searchTerms[0] + "%"})`
      ),
      limit: 10
    });
    return vendors3;
   ...
   */
  extractSearchTerms(vendorName) {
    const cleaned = vendorName.replace(/\s*(주식회사|주|유한회사|유|법인|산업|건설|전기|설비|엔지니어링|ENG|Co\.|Ltd\.|Inc\.|Corp\.)\s*/gi, "").trim();
  ...
  calculateSimilarity(name1, name2) {
    ...
    return Math.round(
      levenshteinSim * 0.3 + jaroWinklerSim * 0.4 + tokenSim * 0.3
    );
  }
  /**
  ...
  normalizeForComparison(name) {
    return name.toLowerCase().replace(/\s*(주식회사|주|유한회사|유|법인|㈜|\(주\)|\(유\))\s*/g, "").replace(/[^a-z0-9가-힣]/g, "").trim();
  }
  /**
   ...
   */
  async createMapping(originalName, vendorId, vendorName, confidence, userId) {
    ...
    if (existing) {
      if (confidence > Number(existing.confidence)) {
        await db.update(vendorMappings).set({
          mappedVendorId: vendorId,
          mappedVendorName: vendorName,
          confidence: confidence.toString(),
          lastUsedAt: /* @__PURE__ */ new Date()
        }).where(eq29(vendorMappings.id, existing.id));
      }
      return;
    }
   ...
   * Update usage count for a mapping
   */
  async updateMappingUsageCount(mappingId) {
    await db.update(vendorMappings).set({
      usageCount: sql15`${vendorMappings.usageCount} + 1`,
      lastUsedAt: /* @__PURE__ */ new Date()
    }).where(eq29(vendorMappings.id, mappingId));
  }
  /**
   * Clear cache
   */
   ...
   */
  async learnFromSelection(originalName, selectedVendorId, selectedVendorName, userId) {
    await this.createMapping(
      originalName,
      selectedVendorId,
      selectedVendorName,
      95,
      // High confidence for user selections
      userId
    );
...
var DuplicateDetectionService = class {
   ...
   * 엑셀 데이터의 중복 검사 수행
   */
  async detectDuplicates(data, sessionId, options = {}) {
    const {
      checkExisting = true,
    ...
    const processedHashes = /* @__PURE__ */ new Map();
    const similarGroups = /* @__PURE__ */ new Map();
   ...
   */
  async findSimilarItems(targetRow, allRows, targetIndex, threshold) {
    const matches = [];
   ...
   */
  suggestMergeStrategy(row, matches) {
   ...
   */
  async checkAgainstDatabase(rows, results) {
    try {
      const recentOrders = await db.select().from(purchaseOrderItems).limit(1e3);
   ...
   */
  findExistingMatch(row, existingItems) {
    let bestMatch = null;
    ...
    for (const item of existingItems) {
      const similarity = this.calculateSimilarity(row, {
        ...
        \uB2E8\uAC00: item.unitPrice,
        \uAC70\uB798\uCC98: item.vendorId,
        // This would need vendor name lookup
        \uD604\uC7A5: item.projectId
        // This would need project name lookup
      });
   ...
   */
  async storeDuplicateGroups(sessionId, groups) {
   ...
   */
  async mergeDuplicates(duplicates, strategy = "manual") {
    const mergedData = [];
    const processedIndices = /* @__PURE__ */ new Set();
    for (const [index2, result] of duplicates.entries()) {
      if (processedIndices.has(index2)) continue;
      if (result.isDuplicate && result.mergeStrategy) {
        switch (result.mergeStrategy.action) {
          ...
          case "merge":
            ...
            break;
          case "replace":
            mergedData.push({ index: index2, action: "replace" });
            processedIndices.add(index2);
            break;
          case "create_new":
            mergedData.push({ index: index2, action: "create" });
            processedIndices.add(index2);
            break;
        }
      } else {
...
var SmartUploadService = class {
   ...
   */
  async processFileAsync(sessionId, buffer, userId) {
    try {
      const parsedData = await this.parseExcelFile(buffer);
      await db.update(validationSessions).set({ totalItems: parsedData.length }).where(eq30(validationSessions.id, sessionId));
      ...
      }).where(eq30(validationSessions.id, sessionId));
      return results;
    } catch (error) {
      await db.update(validationSessions).set({
        status: "failed",
  ...
  async parseExcelFile(buffer) {
    const workbook = XLSX10.read(buffer, { type: "buffer" });
    const sheetName = workbook.SheetNames[0];
    const worksheet = workbook.Sheets[sheetName];
    const jsonData = XLSX10.utils.sheet_to_json(worksheet, { header: 1 });
    ...
    for (let i = 0; i < dataRows.length; i++) {
      const row = dataRows[i];
      if (!row || row.every((cell) => cell === null || cell === void 0 || cell === "")) {
        continue;
      }
      parsedRows.push({
        rowIndex: i + 2,
        // Excel row number (1-indexed, skip header)
  ...
  createColumnMap(headers) {
    ...
    headers.forEach((header, index2) => {
      if (header && typeof header === "string") {
        map.set(header.toLowerCase().trim(), index2);
      }
    });
    return map;
  }
   ...
   */
  getCellValue(row, columnMap, possibleNames) {
    for (const name of possibleNames) {
      const index2 = columnMap.get(name.toLowerCase());
      if (index2 !== void 0 && row[index2] !== void 0 && row[index2] !== null) {
        return String(row[index2]).trim();
      }
    }
    return void 0;
  }
   ...
   */
  getNumericValue(row, columnMap, possibleNames) {
    const value = this.getCellValue(row, columnMap, possibleNames);
    if (value === void 0) return void 0;
    const numeric = parseFloat(value.replace(/[^0-9.-]/g, ""));
    return isNaN(numeric) ? void 0 : numeric;
  }
  /**
   * Validate all rows and store results
   */
  async validateRows(sessionId, rows, userId) {
    let validItems = 0;
    let warningItems = 0;
    let errorItems = 0;
    const validationResultsToInsert = [];
    const duplicateResults = await this.duplicateDetectionService.detectDuplicates(
      rows,
      sessionId,
      {
        checkExisting: false,
        // Temporarily disable database check
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (duplicateCheck && duplicateCheck.isDuplicate) {
        if (duplicateCheck.duplicateType === "exact") {
          ...
          validationWarnings.push(
            `\uC720\uC0AC \uD56D\uBAA9 \uBC1C\uACAC (${duplicateCheck.confidence}% \uC77C\uCE58): \uD589 ${duplicateCheck.matches[0].rowIndex + 1}`
          );
        }
      ...
      for (const field of fieldsToValidate) {
        validationResultsToInsert.push({
          ...
          fieldName: field,
          originalValue: String(row[field] || ""),
          validatedValue: String(row[field] || ""),
          validationStatus: status,
  ...
  async finalizeSession(sessionId, userId) {
    const session2 = await db.query.validationSessions.findFirst({
      where: and23(
        eq30(validationSessions.id, sessionId),
        eq30(validationSessions.userId, userId)
      ),
      with: {
        results: true
      }
   ...
   */
  async getValidatedData(sessionId) {
    const results = await db.query.validationResults.findMany({
      where: eq30(validationResults.sessionId, sessionId)
    });
    const groupedByRow = /* @__PURE__ */ new Map();
    for (const result of results) {
  ...
  groupByProjectAndVendor(data) {
    ...
    for (const row of data) {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          projectId: 1,
          // TODO: Resolve actual project ID
          vendorId: 1,
          // TODO: Resolve actual vendor ID
   ...
   */
  async cleanupSession(sessionId, userId) {
    const session2 = await db.query.validationSessions.findFirst({
      where: and23(
        eq30(validationSessions.id, sessionId),
        eq30(validationSessions.userId, userId)
    ...
    await db.delete(validationResults).where(eq30(validationResults.sessionId, sessionId));
    await db.delete(validationSessions).where(eq30(validationSessions.id, sessionId));
  }
};
...
var AISuggestionService = class {
   ...
   */
  async generateSuggestions(errors, options = {}) {
    const {
   ...
   */
  async generateVendorSuggestion(error) {
   ...
   */
  async generateCategorySuggestion(error) {
    const itemName = error.originalValue;
   ...
   */
  generateNumericSuggestion(error) {
    ...
    if (result.parsed !== void 0) {
      return {
        ...
        // Using 'duplicate' as generic type
        confidence: 95,
        reason: "\uC22B\uC790 \uD615\uC2DD \uC815\uADDC\uD654"
      };
    }
   ...
   */
  async enhanceWithOpenAI(errors, basicSuggestions) {
   ...
   */
  async storeSuggestions(sessionId, suggestions) {
   ...
   */
  async applySuggestions(sessionId, suggestionIds, userId) {
    let applied = 0;
    let failed = 0;
    for (const id of suggestionIds) {
      try {
        const suggestion = await db.query.aiSuggestions.findFirst({
          where: eq31(aiSuggestions.id, id)
        });
        ...
        await db.update(validationResults).set({
          ...
          confidence: suggestion.confidence
        }).where(eq31(validationResults.sessionId, sessionId)).where(eq31(validationResults.rowIndex, suggestion.rowIndex)).where(eq31(validationResults.fieldName, suggestion.fieldName));
   ...
   */
  async getSessionSuggestions(sessionId) {
    return db.query.aiSuggestions.findMany({
      where: eq31(aiSuggestions.sessionId, sessionId)
    });
  }
  /**
   * Clear session suggestions
   */
  async clearSessionSuggestions(sessionId) {
    await db.delete(aiSuggestions).where(eq31(aiSuggestions.sessionId, sessionId));
  }
};
...
var UploadResponseSchema = z10.object({
  ...
  totalItems: z10.number(),
  validItems: z10.number(),
  warningItems: z10.number(),
  errorItems: z10.number(),
  message: z10.string().optional()
});
var ValidationStatusSchema = z10.object({
  sessionId: z10.string(),
  status: z10.enum(["pending", "processing", "completed", "failed"]),
  progress: z10.number(),
  results: z10.array(z10.object({
    rowIndex: z10.number(),
    status: z10.enum(["valid", "warning", "error"]),
    errors: z10.array(z10.string()).optional(),
    warnings: z10.array(z10.string()).optional()
  })).optional()
});
var CorrectionRequestSchema = z10.object({
  sessionId: z10.string(),
  corrections: z10.array(z10.object({
    rowIndex: z10.number(),
    field: z10.string(),
    value: z10.any()
  }))
});
var AISuggestRequestSchema = z10.object({
  sessionId: z10.string(),
  includeCategories: z10.boolean().default(true),
  includeVendors: z10.boolean().default(true),
  includeEmails: z10.boolean().default(true),
  confidenceThreshold: z10.number().min(0).max(100).default(80)
});
var smartUploadService = new SmartUploadService();
var validationService = new ValidationService();
var aiSuggestionService = new AISuggestionService();
var wsService = WebSocketService.getInstance();
router36.post("/upload/smart", requireAuth, upload6.single("file"), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: "No file uploaded" });
    }
    ...
    smartUploadService.processFileAsync(sessionId, req.file.buffer, userId).then((result) => {
...
router36.get("/validation/:sessionId", requireAuth, async (req, res) => {
  try {
    ...
    const session2 = await db.query.validationSessions.findFirst({
      where: (sessions2, { and: and24, eq: eq33 }) => and24(
        eq33(sessions2.id, sessionId),
        eq33(sessions2.userId, userId)
      )
    });
    if (!session2) {
    ...
    const results = await db.query.validationResults.findMany({
      where: (results2, { eq: eq33 }) => eq33(results2.sessionId, sessionId)
    });
    const groupedResults = results.reduce((acc, result) => {
      ...
      acc[status].push({
        rowIndex: result.rowIndex,
        status,
        errors: result.errorMessage ? [result.errorMessage] : void 0,
        warnings: result.suggestion ? [result.suggestion] : void 0
      });
      return acc;
    }, {});
    const response = ValidationStatusSchema.parse({
      ...
      progress: session2.status === "completed" ? 100 : session2.status === "processing" ? 50 : 0,
...
router36.patch("/correction/:sessionId", requireAuth, async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user?.id;
    if (!userId) {
...
router36.post("/ai/suggest", requireAuth, async (req, res) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: "User not authenticated" });
...
router36.post("/finalize/:sessionId", requireAuth, async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user?.id;
    if (!userId) {
    ...
    await db.update(validationSessions).set({
      status: "completed",
      completedAt: /* @__PURE__ */ new Date()
    }).where(eq32(validationSessions.id, sessionId));
...
router36.delete("/session/:sessionId", requireAuth, async (req, res) => {
  try {
    const { sessionId } = req.params;
    const userId = req.user?.id;
    if (!userId) {
...
var excel_smart_upload_default = router36;
...
router37.use("/api/excel", excel_smart_upload_default);
